 ...
 * @Last Modified time: 2019-05-27 09:17:14
 */
<template>
  <div class="app-container">
    ...
    <el-card v-if="active === 0" class="box-card">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="120px" style="margin-left:50px;">
        <el-form-item label="大牌任务标题" prop="title" style="width: 400px">
          <el-input v-model="dataForm.title"/>
        </el-form-item>
        <el-form-item label="大牌任务描述" prop="description" style="width: 400px">
          <el-input v-model="dataForm.description" :autosize="true" type="textarea"/>
        </el-form-item>
        <el-form-item label="大牌任务排序" prop="sort" style="width: 400px">
    ...
    <el-card v-if="active === 1" class="box-card">
      ...
      <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
        ...
        <!-- <el-table-column align="center" label="收益（元）" prop="taskRate"/> -->
        <el-table-column align="center" label="收益（元）" prop="taskRate">
          <template slot-scope="scope">
            <span>{{ scope.row.taskRate }}/任务</span>
          </template>
        </el-table-column>
...
import { queryTaskConfigsByBizId, editTopMission, updateTopMission } from '@/api/operation/topMission'
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      ...
      editTopMission(para).then((res) => {
        this.dataForm = res.data.data.taskSpecialConfig
    ...
    confirmAddMission() {
      ...
      updateTopMission(para).then((res) => {
        this.$router.push({
