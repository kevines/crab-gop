...
<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-form ref="searchForm" :model="searchForm" :inline="true">
        <el-form-item label="兑换码" prop="ticketNo">
          <el-input v-model="searchForm.ticketNo" clearable class="filter-item" style="width: 200px;"
            placeholder="兑换码" />
        </el-form-item>

        <el-form-item label="联系人姓名" prop="userName">
          <el-input v-model="searchForm.userName" clearable class="filter-item" style="width: 200px;"
            placeholder="联系人姓名" />
        </el-form-item>

        <el-form-item label="联系电话" prop="mobile">
          <el-input v-model="searchForm.mobile" clearable class="filter-item" style="width: 200px;"
            placeholder="联系电话" />
        </el-form-item>

        <el-form-item label="物流单号" prop="logisticsNo">
          <el-input v-model="searchForm.logisticsNo" clearable class="filter-item" style="width: 200px;"
            placeholder="物流单号" />
        </el-form-item>
    ...
    <!-- 查询结果 -->
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="更新列表中......" border fit
      ...
      <el-table-column align="center" label="兑换状态" prop="isExchange">
        <template slot-scope="scope">
          <el-tag :type="scope.row.exchangeTicket.isExchange ? 'success' : 'danger'">{{
              scope.row.exchangeTicket.isExchange
                ? '已兑换' : '未兑换'
          }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="发货状态" prop="isSend">
        <template slot-scope="scope">
          <el-tag :type="scope.row.exchangeTicket.isSend ? 'success' : 'danger'">{{ scope.row.exchangeTicket.isSend ?
              '已发货'
              : '未发货'
          }}</el-tag>
        </template>
      ...
      <el-table-column align="center" label="兑换日期" prop="exchangeDate">
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.exchangeTicket.exchangeDate != null">{{ scope.row.exchangeTicket.exchangeDate }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="发货日期" prop="sendDate">
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.exchangeTicket.sendDate != null">{{ scope.row.exchangeTicket.sendDate }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="备注" prop="remark">
        <template slot-scope="scope">
          <span v-if="scope.row.remark === null">暂无</span>
      ...
      <el-table-column align="center" label="操作" width="300" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.exchangeTicket.isSend === false" type="success" size="small"
            @click="handleSendDialog(scope.row)">处理发货</el-button>
          <el-button type="success" size="small" @click="handleSendRemark(scope.row)">备注</el-button>
    ...
    <!-- 处理备注对话框 -->
    <el-dialog :visible.sync="remarkDialogVisible" title="备注" width="500">
      <el-form ref="ticketRemark" :model="ticketRemark" :rules="remarkRules" label-position="left" label-width="100px">
        <el-form-item label="备注" prop="remark">
          <el-input v-model="ticketRemark.remark" placeholder="请输入备注" />
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
import { remark } from "@/api/ticket/record";
...
export default {
  ...
  methods: {
     ...
     * 点击出现处理备注弹框
     */
    handleSendRemark(row) {
      // 打开弹窗
      console.log(row);
      this.remarkDialogVisible = true;
      this.ticketRemark.ticketNo = row.ticketNo;
      console.log(this.ticketRemark);
    },
    /**
     * 备注操作
     */
    handleRemark() {
      this.$refs["ticketRemark"].validate(valid => {
        if (valid) {
          const para = {
            ticketNo: this.ticketRemark.ticketNo,
            remark: this.ticketRemark.remark
          };
          console.info(para);
          remark(para)
            .then(res => {
